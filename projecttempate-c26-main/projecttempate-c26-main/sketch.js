...
function draw() {
 ...
 // Use for loop to display arrow using showArrow() function
    for(var i=0;i<arrowss.length;i++){
      showArrows(arrowss[i],i);
    }
...
function keyPressed() {
  ...
  if(keyCode === 32){
    // create an arrow object and add into an array ; set its angle same as angle of playerArcher
      arrow= new PlayerArrow(playerArcher.body.position.x,playerArcher.body.position.y,40,50);
...
function keyReleased () {
  ...
  if(keyCode === 32){
    ...
    arrowss[arrowss.length-1].shoot();
  }
